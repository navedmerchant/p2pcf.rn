...
// Minimal base64 helpers for ArrayBuffer & back (React Native / JS runtime)
function arrayBufferToBase64(buffer) {
  ...
  throw new Error('No base64 encoder (btoa) available in this environment');
}

function base64ToArrayBuffer(base64) {
  if (typeof atob !== 'function') {
    throw new Error('No base64 decoder (atob) available in this environment');
  }
...
const randomstring = (len) => {
  ...
  if (typeof btoa === 'function') {
    // Base64, strip padding for compactness
    return btoa(binary).replace(/[=]/g, '');
  }
  // Fallback: hex-derived ID (still deterministic from random bytes)
  return arrayBufferToHex(bytes).slice(0, len);
};

// Minimal UTF-8 encode/decode without TextEncoder/TextDecoder

function arrToText(u8) {
  ...
  for (let i = 0; i < u8.length; i++) {
    ...
    } else if (byte1 >= 0xc0 && byte1 < 0xe0 && i + 1 < u8.length) {
      const byte2 = u8[++i];
      out += String.fromCharCode(((byte1 & 0x1f) << 6) | (byte2 & 0x3f));
    } else if (byte1 >= 0xe0 && byte1 < 0xf0 && i + 2 < u8.length) {
      const byte2 = u8[++i];
      const byte3 = u8[++i];
      out += String.fromCharCode(
        ((byte1 & 0x0f) << 12) | ((byte2 & 0x3f) << 6) | (byte3 & 0x3f)
      );
    } else if (byte1 >= 0xf0 && i + 3 < u8.length) {
      const byte2 = u8[++i];
      const byte3 = u8[++i];
      const byte4 = u8[++i];
      let codepoint =
...
const ua = ''; // React Native: no browser UA
const iOS = false;
const webkit = false;
const iOSSafari = false;
...
export default class P2PCF extends EventEmitter {
  constructor(clientId = '', roomId = '', options = {}) {
    ...
    // ContextID: in React Native/non-browser envs we can't rely on window.history.
    // Use a randomly generated ID scoped to this instance instead.
  ...
  async start() {
    ...
    // In non-browser environments (React Native), there is no window/unload.
    // Consumers should call destroy() explicitly when appropriate.
  }
  ...
  destroy() {
    ...
    // No browser-specific unload listeners in this React Native-targeted build.
